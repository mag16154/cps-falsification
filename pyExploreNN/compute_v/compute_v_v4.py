...
## We increase the simulation time in accordance to every increase in the jump_size
# so as to keep the number of input/output pairs same across each jump_size run
...
class nnOnStateSpace(NNConfiguration):
    ...
    def createNN(self, jump_size=1):
        ...
        input = []
        output = []
        #print(len(self.trajectories[traj_combs[0][0]]))
        steps = jump_size*1001
#       steps = len(self.trajectories[traj_combs[0][0]])
        for traj_pair in traj_combs:
            ...
            for step in range(0, steps-jump_size, jump_size):
                ...
                x_vp_xp_pair = x_vp_xp_pair + list(traj_1[step+jump_size])
