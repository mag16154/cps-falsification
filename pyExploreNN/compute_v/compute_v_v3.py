    ...
    def createNN(self, jump_size=1):
        ...
        for traj_pair in traj_combs:
            ...
            for step in range(0, steps, jump_size):
                x_vp_xp_t_pair = list(traj_1[step])
                v_val = traj_2[step] - traj_1[step]
                t_idx = random.randint(0, steps-step)
                t_val = step+t_idx
                vprime_val = traj_2[t_val] - traj_1[t_val]
                x_vp_xp_t_pair = x_vp_xp_t_pair + list(vprime_val)
                x_vp_xp_t_pair = x_vp_xp_t_pair + list(traj_1[t_val])
                x_vp_xp_t_pair.append(t_val)
                #print(x_v_xprime_t_pair)
                input.append(x_vp_xp_t_pair)
                output.append(v_val)
        print(len(input))
