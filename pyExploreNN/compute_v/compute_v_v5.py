    ...
    def createNN(self, jump_size=1):
        ...
        for traj_pair in traj_combs:
            ...
            for step in range(1, steps, jump_size):
                x_idx = 0
                x_vp_t_pair = list(traj_1[x_idx])
                v_val = traj_2[x_idx] - traj_1[x_idx]
                t_val = step
                # t_idx = random.randint(0, steps-step)
                # t_val = step+t_idx
                vprime_val = traj_2[t_val] - traj_1[t_val]
                x_vp_t_pair = x_vp_t_pair + list(vprime_val)
                x_vp_t_pair = x_vp_t_pair + [t_val*self.timeStep]
                input.append(x_vp_t_pair)
                output.append(v_val)
        print(len(input))
        ...
        for traj_pair in traj_combs:
            ...
            for step in range(1, steps, jump_size):
                ...
                x_vp_t_pair = x_vp_t_pair + list(vprime_val)
                x_vp_t_pair = x_vp_t_pair + [t_val*self.timeStep]
                x_test_local.append(x_vp_t_pair)
                y_test_local.append(v_val)
