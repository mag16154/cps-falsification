...
%% at every time step
beta = zeros(beta_time_counts, no_of_dimensions, no_of_elements);
for idt = 1:size(traj_t,1);
	if (base_vectors_ignore(idt) == 1);
		continue;
	end
...
max_beta_array = zeros(beta_time_counts, no_of_dimensions);
...
figure(1);
plot(beta_time_span, max_beta_array(:,:));
xlabel('Time'),
ylabel('Beta'),
