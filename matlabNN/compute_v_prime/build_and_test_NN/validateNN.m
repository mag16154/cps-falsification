...
v_prime_vals = zeros(no_of_samples, no_of_dims, time_steps-1);
...
if layers == 1
	for idx = 1:no_of_samples
		...
		for idy=1:(time_steps-1)
			...
			h1_layer_input = [x_val v_val x_prime_val]';
			...
			v_prime_vals(idx, :, idy) = v_prime_val;
			o_layer_output = purelin(IW_times_o_input_plus_bias);
			o_layer_output_vals(idx, :, idy) = o_layer_output(:);
			v_mae(idx, idy) = mae(o_layer_output_vals(idx, :, idy) - v_prime_vals(idx, :, idy));
		end
	end
elseif layers == 2
	for idx = 1:no_of_samples
...
v_mae = mse(o_layer_output_vals(:, 1:no_of_dims, :), v_prime_vals(:, 1:no_of_dims, :));
end
