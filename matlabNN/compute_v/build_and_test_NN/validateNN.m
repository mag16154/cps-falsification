function [v_sum] = validateNN(net, traj_x, time_steps, no_of_dims, traj_combs, layers, preprocess) 
...
v_rae = zeros(no_of_samples, time_steps-1);
if layers == 1
	for idx = 1:no_of_samples
		...
		for idy=1:(time_steps-1)
			...
			h1_layer_input = [x_val x_prime_val v_prime_val t_val]';
			...
			v_rae(idx, idy) = norm(v_val - o_layer_output) ./ norm(v_val);
		end
	end
elseif layers == 2
	for idx = 1:no_of_samples
...
%v_mae = mse(o_layer_output_vals(:, 1:no_of_dims, :), v_vals(:, 1:no_of_dims, :));
v_sum = sum(v_rae)./no_of_samples;
v_sum = sum(v_sum)./time_steps;
end
